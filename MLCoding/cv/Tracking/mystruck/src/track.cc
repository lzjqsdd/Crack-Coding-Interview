...
//		cout<<"track score: "<<maxscore<<endl;
		UpdateLearner(m_img);
	}
	draw(c_img,m_bb);
}
...
void Track::Initialise (IplImage *img,FloatRect bb)
{
	m_bb = FloatRect(bb.x,bb.y,bb.width,bb.height);
	m_bb.ToInt();
	UpdateLearner(img);
}
...
void Track::UpdateLearner(IplImage *img)
{
	...
	FloatRect imgRect(0,0,img->width,img->height);
