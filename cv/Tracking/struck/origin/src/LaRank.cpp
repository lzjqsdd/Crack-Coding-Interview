...
{
	//此时传递的是sample表示（当前处理的图片，以及每个样本的位置rect)
	//y默认就传递的0,中心的样本（也是正样本）存在rect列表中第一个
	// add new support pattern
	SupportPattern* sp = new SupportPattern;
	const vector<FloatRect>& rects = sample.GetRects(); //得到采样的样本位置Rect
	FloatRect centre = rects[y]; //传递0表示中心的Rect
	for (int i = 0; i < (int)rects.size(); ++i)
	{
		// express r in coord frame of centre sample
		FloatRect r = rects[i];
		r.Translate(-centre.XMin(), -centre.YMin()); //计算变换y，每个样本的左上角减去正样本的左上角坐标
		sp->yv.push_back(r);//把可能的所有的变换记录到support pattern
		//这段主要展示细节运行过程
	...
	const_cast<Features&>(m_features).Eval(sample, sp->x); //计算得到的Feature放到sp->中,(x,y)是一个支持向量
	cout<<"sp->y的值-->"<<y<<endl;
	sp->y = y;//y貌似总是0
	sp->refCount = 0;
	m_sps.push_back(sp); //每次把supportPattern支持模式加入列表

	//更新参数
	//(这里是最主要的部分)
	//Learner找到当前帧中的目标位置，以该目标位置为中心进行采样，然后update,所以这个目标位置在训练中一定是正样本，
